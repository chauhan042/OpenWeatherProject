...
struct BulkWeatherDayDataContainer: Codable {
  var errorDataDTO: ErrorDataDTO?
  var weatherInformationDTOs: [WeatherListDTO]?
}
...
struct WeatherDataManagerStoredContentsWrapper: Codable {
  ...
  var listWeatherDataObject: BulkWeatherDayDataContainer?
}
...
final class WeatherDataService {
  ...
  private(set) var listWeatherDataObject: BulkWeatherDayDataContainer?
  ...
  func update(withCompletionHandler completionHandler: ((UpdateStatus) -> Void)?) {
    ...
    fetchWeatherDataBackgroundQueue.async {
      ...
      WeatherNetworkingService.shared.fetchBulkWeatherInformation(completionHandler: { weatherData in
        ...
        dispatchGroup.enter()
        WeatherNetworkingService.shared.fetchListWeatherInformation(completionHandler: { (listWeatherData) in
            listWeatherDataObject = listWeatherData
            dispatchGroup.leave()
        })
      ...
      if nearbyWeatherDataObject != nil {
        ...
        if listWeatherDataObject != nil {
          self.listWeatherDataObject = listWeatherDataObject
        }
  ...
  func weatherDTO(forIdentifier identifier: String) -> WeatherInformationDTO? {
    if let bookmarkedLocationMatch = bookmarkedWeatherDataObjects?.first(where: {
      return $0.weatherInformationDTO?.cityName == identifier
    ...
    if let nearbyLocationMatch = nearbyWeatherDataObject?.weatherInformationDTOs?.first(where: { weatherDTO in
      return weatherDTO.cityName == identifier
    }) {
...
extension WeatherDataService: DataStorageProtocol {
  ...
  static func loadData() -> WeatherDataService? {
    ...
    weatherService.listWeatherDataObject = weatherDataManagerStoredContents.listWeatherDataObject
  ...
  static func storeData() {
    ...
    weatherServiceBackgroundQueue.async {
      let weatherDataManagerStoredContents = WeatherDataManagerStoredContentsWrapper(
        ...
        listWeatherDataObject: WeatherDataService.shared.listWeatherDataObject
      )
