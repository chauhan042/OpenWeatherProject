...
final class WeatherNetworkingService {
  ...
  func beginListeningNetworkReachability() {
    reachabilityManager?.startListening(onUpdatePerforming: { [weak self](status) in
        ...
        case .reachable(.ethernetOrWiFi), .reachable(.cellular): self?.reachabilityStatus = .connected
        }
  ...
  func fetchBulkWeatherInformation(completionHandler: @escaping (BulkWeatherDataContainer) -> Void) {
    ...
    AF
      .request(
        Constants.Urls.kOpenWeatherMapMultiStationtDataRequestUrl(with: currentLatitude, currentLongitude: currentLongitude),
        method: .get,
        parameters: nil,
        encoding: JSONEncoding.default,
        headers: nil
  ...
  private func extractBulkWeatherInformation(_ data: Data) -> BulkWeatherDataContainer {
    ...
    func fetchListWeatherInformation(completionHandler: @escaping (BulkWeatherDayDataContainer) -> Void) {
      guard let currentLatitude = UserLocationService.shared.currentLatitude, let currentLongitude = UserLocationService.shared.currentLongitude else {
        let errorDataDTO = ErrorDataDTO(errorType: ErrorDataDTO.ErrorType(value: .locationUnavailableError), httpStatusCode: nil)
        return completionHandler(BulkWeatherDayDataContainer(errorDataDTO: errorDataDTO, weatherInformationDTOs: nil))
      }
      
      
      AF
        .request(
          Constants.Urls.kOpenWeatherMapForeCastUrl(with: currentLatitude, currentLongitude: currentLongitude),
          method: .get,
          parameters: nil,
          encoding: JSONEncoding.default,
          headers: nil
        ...
        .responseData(queue: weatherFetchQueue) { [weak self] response in
          ...
          completionHandler(self.extractDayWeatherInformation(data))
      }
    }
    private func extractDayWeatherInformation(_ data: Data) -> BulkWeatherDayDataContainer {
      guard let multiWeatherData = try? JSONDecoder().decode(WeatherInformationDayWrapper.self, from: data) else {
        let errorDataDTO = ErrorDataDTO(errorType: ErrorDataDTO.ErrorType(value: .jsonSerializationError), httpStatusCode: nil)
        return BulkWeatherDayDataContainer(errorDataDTO: errorDataDTO, weatherInformationDTOs: nil)
      }
      return BulkWeatherDayDataContainer(errorDataDTO: nil, weatherInformationDTOs: multiWeatherData.list)
    }
}
